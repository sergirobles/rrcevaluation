...
ClassVisualization.prototype.load_visualization = function(){
    ...
    var html_iou = "";
    if(sampleData.iouMat==undefined){
        html_iou = "<div class='alert alert-warning'>The parameter <b>iouMat</b> is not present on the sample results file</div>";
    }else{
    ...
    if (this.sampleData.gtPolPoints==undefined){
        evalLog = "<div class='alert alert-warning'>The parameter <b>gtPolPoints</b> is not present on the sample results file</div>" + evalLog;
    }
    if (this.sampleData.detPolPoints==undefined){
        evalLog = "<div class='alert alert-warning'>The parameter <b>detPolPoints</b> is not present on the sample results file</div>" + evalLog;
   }      
    ...
    for (var i=0;i<this.sampleData.gtPolPoints.length;i++){
        ...
        //if (bb.id_s==current_id_submit){
            ...
            if (bb.length==4){
                ...
                for(var p = 1;p<bb.length/2;p++){
                    this.ctx_gt.lineTo(this.original_to_zoom_val(parseInt(bb[p*2])), this.original_to_zoom_val_y(parseInt(bb[p*2+1])));
                }                
    ...
    for (var i=0;i<this.sampleData.detPolPoints.length;i++){
        ...
        var type = this.sampleData.detTypes[i];
            ...
            if (bb.length==4){
                ...
                for(var p = 1;p<bb.length/2;p++){
                    this.ctx_det.lineTo(this.original_to_zoom_val(parseInt(bb[p*2])), this.original_to_zoom_val_y(parseInt(bb[p*2+1])));
                }  
